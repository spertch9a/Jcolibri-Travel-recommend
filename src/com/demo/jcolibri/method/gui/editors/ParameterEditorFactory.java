...
public class ParameterEditorFactory {
	...
	ParameterEditorFactory.registerEditor(Enum.class, jcolibri.method.gui.editors.EnumEditor.class);
     ...
     */
    public static jcolibri.method.gui.editors.ParameterEditor getEditor(Class<?> type) {
	try
	{
	    
	    Class editor = table.get(type);
	    if(editor != null)
		return (jcolibri.method.gui.editors.ParameterEditor)table.get(type).newInstance();
	    
	    for(Class<?> c : table.keySet())
		if(c.isAssignableFrom(type))
		    editor = table.get(c);
	    
	    if(editor.equals(jcolibri.method.gui.editors.EnumEditor.class))
		return new EnumEditor(type);
	    if(editor != null)
		return (jcolibri.method.gui.editors.ParameterEditor)editor.newInstance();
     ...
     * Creates the editor and configures it with its data-type name
     */
    public static jcolibri.method.gui.editors.ParameterEditor getEditor(Class<?> type, Collection<Object> allowedValues)
    {
