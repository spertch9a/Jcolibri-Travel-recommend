package com.demo.jcolibri.method.retrieve.NNretrieval.similarity.local.textual.compressionbased;
 ...
 *
 * Compression Based Dissimilarity (CDM) is defined as follows
 * CDM(x, y) = C(xy) / (C(x) + C(y))
 * where C(x) is the size of string x after compression (and C(y) similarly)
 * and C(xy) is the size, after compression, of the string that comprises y 
 ...
 * CDM(x, y) = 0 iff x = y
 * CDM(x, y) = CDM(y, x)
 * CDM(x, y) + CDM(y, z) >= CDM(x, z)
 * Do not use this measure in any application that requires metric properties.
 * 
 * Values of CDM will lies in the range (0.5, 1.0].
 *
 * We convert to a similarity measure by returning 1.0 - CDM(x, y).
 *
 * This similarity measure is effective in case-based spam filtering using GZip and
 * PPM text compression algorithms. For the details, see:
 * <pre>
 ...
 * }
 * </pre>
 * Its effectiveness outside of spam filtering (e.g. texts other than emails, short strings,
 * images, etc.) has not been demonstrated. To be effective, the 'right' compression algorithm
 * must be used.
 *
 * @author Derek Bridge
 * 18/05/07 
...
{
        private jcolibri.method.retrieve.NNretrieval.similarity.local.textual.compressionbased.ICompressor compressor;

        /**
         * @param compressor an object that encapsulates the compression algorithm to be used.
         */
